"""
metrics/metrics/feature_attribution/timeseries_multivariate/robustness/continuity.py

Continuity for feature attribution on multivariate time series.
"""
...
from metrics.utils.ts_helpers import spearman_rank_corr, ts_class_scores, ts_regression_scores


@register_metric("fa_ts_mv_continuity")
class FAMVContinuity(BaseFeatureAttributionMetric):
    """
    Continuity for multivariate time-series attributions.

    Measures how smoothly the attribution map changes under small input
    perturbations that leave the model's prediction (approximately) unchanged.
    ...
    Per-sample score = 1 - mean( Spearman ρ(vec(E), vec(Ê)) ) over M accepted
    perturbations, where E is the original attribution map and Ê is the
    attribution on the perturbed input (recomputed via explain_fn).

    Score close to 0 → high Spearman ρ → explanation is stable / continuous.
    Score close to 1 → low Spearman ρ → explanation changes significantly.
    ...
    METADATA = MetricMetadata(
        metric_id="fa_ts_mv_continuity",
        display_name="FA-TS — Continuity",
        category="Robustness",
    ...
    def evaluate_attributions(
        ...
        **kwargs,
        ...
        """
        Compute Continuity for a batch of (x, attribution) pairs.
        ...
        for b in range(B):
            ...
            while collected < self.n_perturbations and attempts < max_attempts:
                ...
                if accepted:
                    ...
                    rhos.append(spearman_rank_corr(Eb, Et))
                    collected += 1

            if rhos:
                # Score = 1 - mean(ρ): low continuity score means explanations are stable
                scores[b] = 1.0 - float(np.mean(rhos))
